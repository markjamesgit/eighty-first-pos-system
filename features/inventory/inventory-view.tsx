...
import { Archive, Plus, Minus, Search, SlidersHorizontal } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden rounded-2xl border-stone-200 bg-white shadow-xl shadow-stone-200/40">
        <CardHeader className="bg-stone-50/80 backdrop-blur-md pb-6 border-b border-stone-100">
          <CardTitle className="text-lg font-black text-stone-900 tracking-tight">Inventory Management</CardTitle>
          <CardDescription className="text-stone-500 font-medium text-xs">Real-time tracking of raw materials and ingredients.</CardDescription>
        </CardHeader>
        <CardContent className="grid p-0 lg:grid-cols-[260px_1fr]">
          <aside className="space-y-5 border-b border-stone-100 bg-stone-50/60 p-5 lg:border-b-0 lg:border-r">
            <div>
              ...
              <p className="mt-1 text-xs text-stone-500">Filter stock and monitor critical inventory levels.</p>
            </div>
            ...
            <div className="space-y-2">
              <div className="rounded-xl border border-stone-200 bg-white p-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Total Ingredients</p>
                <p className="mt-1 text-2xl font-black text-stone-900">{ingredients.length}</p>
              </div>
              <div className="rounded-xl border border-stone-200 bg-white p-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Low Stock Alerts</p>
                <p className="mt-1 text-2xl font-black text-stone-900">{lowStockCount}</p>
              </div>
              <div className="rounded-xl border border-stone-200 bg-white p-3">
                <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Active Ingredients</p>
              ...
              <div className="rounded-xl border border-stone-200 bg-white p-3">
                ...
                <p className="mt-1 text-2xl font-black text-stone-900">
                  ...
                  <span className="ml-1 text-sm font-semibold text-stone-400">({filteredLowStockCount} low)</span>
                </p>
              </div>
            </div>
          </aside>
          ...
          <Table>
            <TableHeader className="bg-stone-50/30">
              <TableRow className="hover:bg-transparent border-stone-100 h-14">
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 pl-8">Ingredient</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 text-center">In Stock</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 text-center">Unit</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 text-center">Status</TableHead>
                <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400">Usage context</TableHead>
                <TableHead className="text-right text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 pr-8">Adjustments</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={6} className="h-64 text-center text-stone-400 font-medium italic animate-pulse text-xs tracking-widest uppercase opacity-40">Synchronizing Warehouse...</TableCell></TableRow>
                    ...
                    <TableRow key={item.id} className={cn("hover:bg-stone-50/50 transition-all border-stone-100", isLow && "bg-amber-50/20")}>
                      <TableCell className="pl-8 py-5">
                        <div className="flex flex-col">
                          <span className="font-black text-stone-900 text-sm tracking-tight">{item.name}</span>
                          <span className="text-[9px] font-bold uppercase tracking-widest text-stone-400 mt-1">ID: {item.id.slice(0, 8)}</span>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="text-right pr-8">
                        <div className="flex items-center justify-end gap-3">
                          <div className="flex items-center rounded-xl border border-stone-200 bg-white p-0.5 shadow-sm overflow-hidden ring-1 ring-stone-100/50">
                            ...
                            <span className={cn("w-12 text-center text-xs font-black", adjValue !== 0 ? "text-stone-950" : "text-stone-200")}>
                          ...
                          <Button
                            ...
                            className="h-9 px-5 text-[10px] font-black uppercase tracking-widest bg-stone-900 text-stone-50 hover:bg-stone-800 transition-all shadow-md active:scale-95"
                          >
                            Commit
                          </Button>
                        </div>
