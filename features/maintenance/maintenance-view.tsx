    ...
    <div className="space-y-8">
      ...
      <div>
        <Card className="overflow-hidden rounded-2xl border-stone-200 bg-white shadow-lg shadow-stone-200/50">
          <CardHeader className="border-b border-stone-100 bg-stone-50/80 pb-6">
            <CardTitle className="text-xl font-black tracking-tight text-stone-900">{sectionConfig.title} Registry</CardTitle>
            <CardDescription className="font-medium text-stone-500">Manage your maintenance records and metadata.</CardDescription>
          </CardHeader>
          <CardContent className="grid p-0 lg:grid-cols-[260px_1fr]">
            <aside className="space-y-5 border-b border-stone-100 bg-stone-50/60 p-5 lg:border-b-0 lg:border-r">
              <div>
                ...
                <p className="mt-1 text-xs text-stone-500">Search and monitor your maintenance records.</p>
              </div>
              ...
              <div className="space-y-2">
                <div className="rounded-xl border border-stone-200 bg-white p-3">
                  <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Total Records</p>
                  <p className="mt-1 text-2xl font-black text-stone-900">{items.length}</p>
                </div>
                <div className="rounded-xl border border-stone-200 bg-white p-3">
                  <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">Active Records</p>
                  <p className="mt-1 text-2xl font-black text-stone-900">{activeCount}</p>
                </div>
                <div className="rounded-xl border border-stone-200 bg-white p-3">
                  <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">With Description</p>
                  <p className="mt-1 text-2xl font-black text-stone-900">{filledDescriptionCount}</p>
                </div>
                <div className="rounded-xl border border-stone-200 bg-white p-3">
                  <p className="text-[10px] font-black uppercase tracking-widest text-stone-400">With Media Asset</p>
                  <p className="mt-1 text-2xl font-black text-stone-900">{withAssetCount}</p>
                </div>
              </div>
            </aside>
            <div>
            ...
            <Table>
              <TableHeader className="bg-stone-50/30">
                <TableRow className="hover:bg-transparent border-stone-100 h-14">
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 pl-8">Image</TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 pl-8">Entry Name</TableHead>
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 text-center">Status</TableHead>
                  {(maintenanceSection === "variants" || maintenanceSection === "addons") && (
                    <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 text-center">
                      Description
                    </TableHead>
                  )}
                  <TableHead className="text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 text-center">
                  ...
                  <TableHead className="text-right text-[10px] font-black uppercase tracking-[0.2em] text-stone-400 pr-8">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {loading ? (
                    ...
                    <TableRow key={item.id} className="h-20 border-stone-100 transition-all hover:bg-stone-50/50">
                      ...
                      <TableCell className="pl-8">
                         <div className="flex flex-col">
                            <span className="font-black text-stone-950 text-sm tracking-tight">{item.name}</span>
                         </div>
                      </TableCell>
                      <TableCell className="text-center">
                        <span
                          className={cn(
                            "inline-flex h-6 items-center rounded-full px-2.5 text-[9px] font-black uppercase tracking-widest",
                            item.isActive ? "bg-emerald-100 text-emerald-700" : "bg-stone-200 text-stone-500",
                          )}
                        >
                          {item.isActive ? "Active" : "Inactive"}
                      ...
                      {(maintenanceSection === "variants" || maintenanceSection === "addons") && (
                        <TableCell className="text-center">
                          <span className="line-clamp-1 text-xs font-medium text-stone-500">
                            {item.description || "No supplemental details"}
                          </span>
                        </TableCell>
                      )}
                      <TableCell className="text-center">
                         {(maintenanceSection === "variants" || maintenanceSection === "addons") ? (
                           <span className="font-black text-stone-900 text-sm">+{formatCurrency(Number(item.price))}</span>
                         ) : (
                           <span className="line-clamp-1 text-xs font-medium text-stone-500">
                             {item.description || "No supplemental details"}
                           </span>
      ...
      >
        <DialogContent className="max-h-[90vh] overflow-y-auto rounded-2xl border-stone-200 p-0 sm:max-w-2xl">
          <DialogHeader>
            <div className="border-b border-stone-100 bg-stone-50/70 px-6 py-5">
              ...
              <DialogDescription className="mt-1 text-sm text-stone-500">
                Fill in the fields below to save this maintenance record.
              </DialogDescription>
            </div>
          </DialogHeader>
          <div className="grid gap-6 px-6 pb-6 md:grid-cols-[1fr_220px]">
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-stone-200 bg-white p-4">
                <p className="mb-3 text-[10px] font-black uppercase tracking-widest text-stone-400">Configuration</p>
                {maintenanceSection !== "modifiers" && maintenanceSection !== "categories" ? (
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Price Delta</label>
                    <Input
                      type="number"
                      placeholder="0.00"
                      value={form.price}
                  ...
                  <p className="text-sm text-stone-500">No price delta is required for this section.</p>
                )}
              ...
              <div className="space-y-2 rounded-xl border border-stone-200 bg-stone-50/70 p-4">
                ...
                {form.imageUrl ? (
                  <div className="rounded-lg border border-stone-200 bg-white p-2">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={form.imageUrl} className="h-32 w-full rounded object-cover" alt="" />
                  </div>
                ) : (
            ...
            <div className="flex justify-end gap-2 border-t border-stone-100 pt-4 md:col-span-2">
              <Button
                variant="outline"
              ...
              <Button
                ...
                className="rounded-xl bg-stone-900 px-6 text-white hover:bg-stone-800"
              >
